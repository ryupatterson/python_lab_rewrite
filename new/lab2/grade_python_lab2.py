...
class Lab2UnitTests(unittest.TestCase):
    def test_withdrawal(self):
        ...
        with self.subTest("Successful withdrawal test"):
            ...
            actual = python_lab2.withdrawal(balance, withdrawal)
    ...
    def test_product_is_even(self):
        with self.subTest("Product is even"):
            expected = True
            actual = python_lab2.product_is_even(7, 2)
            self.assertEqual(expected, actual)
        
        with self.subTest("Product is odd"):
            expected = False
            actual = python_lab2.product_is_even(7, 3)
            self.assertEqual(expected, actual)

        with self.subTest("Product is zero"):
            expected = True
            actual = python_lab2.product_is_even(7, 0)
            self.assertEqual(expected, actual)

    def test_reverse_squared(self):
        with self.subTest("Does not contain .reverse()"):
            code = inspect.getsource(python_lab2.reverse_squared)
        ...
        with self.subTest("Generic list reversal test"):
            ...
            actual = python_lab2.reverse_squared(example_input)
    ...
    def test_times_tables(self):
        x = 3
        y = 5
        actual = python_lab2.times_tables(x, y)
    ...
    def test_remove_vowels(self):
        input = "hello"
        expected = "hll"
        actual = python_lab2.remove_vowels(input)
        self.assertEqual(expected, actual)
